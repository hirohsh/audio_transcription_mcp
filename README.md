...
### GitHub Copilot CLI への登録

GitHub Copilot CLI の MCP 設定で以下のように登録します:

- **Type**: local
- **Command**: `/path/to/audio_transcription_mcp/target/release/audio_transcription_mcp`

> **注意**: `ORT_DYLIB_PATH` はバイナリが自動検出するため、通常は設定不要です。
> `lib/libonnxruntime.dylib` が実行ファイルからの相対パス (`../../lib/`) に存在すれば自動的にロードされます。

環境変数を明示的に指定する場合は、以下を設定してください:

| 変数名 | 値の例 |
|--------|--------|
| `MCP_WORK_DIR` | `/path/to/your/audio/files` |
| `MCP_MODEL_DIR` | `/path/to/audio_transcription_mcp/models` |
| `ORT_DYLIB_PATH` | `/path/to/audio_transcription_mcp/lib/libonnxruntime.dylib` |
...
| `ORT_DYLIB_PATH` | ONNX Runtime 共有ライブラリのパス | 自動検出 (実行ファイル相対パス → `/opt/homebrew/lib/`) |
